...
export function setConnectedUi(state, els, connected) {
  ...
  els.refreshPortsBtn.disabled = !getSerialSupport().ok;
...
export function getSerialSupport() {
  if (!window.wattmeterDesktop) {
    return {
      ok: false,
      message: "Desktop serial bridge is unavailable. Open the Electron app.",
    };
  }

  if (
    typeof window.wattmeterDesktop.listSerialPorts !== "function"
    || typeof window.wattmeterDesktop.openSerialPort !== "function"
    || typeof window.wattmeterDesktop.closeSerialPort !== "function"
  ) {
    return {
      ok: false,
      message: "Desktop serial bridge is incomplete. Restart the app.",
    };
  }

  return {
    ok: true,
    message: "Desktop serial ready",
  };
}
...
export async function openDriverHelp(els) {
  if (!window.wattmeterDesktop || typeof window.wattmeterDesktop.openDriverFolder !== "function") {
    setDiagnostics(els, "install the CH340/CH341 driver, then reconnect the Arduino", "error");
    return;
  }

  const result = await window.wattmeterDesktop.openDriverFolder();
  if (result.ok) {
    setDiagnostics(els, "opened bundled driver folder", "ok");
    return;
  }

  setDiagnostics(els, `driver folder unavailable - ${result.error || "missing bundled driver files"}`, "error");
}
